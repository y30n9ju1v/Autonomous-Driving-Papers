...
최근 몇 년 동안 컴퓨터 비전, 머신러닝, 자율주행차와 같은 AI 관련 분야에서 엄청난 발전이 있었습니다. 빠르게 성장하는 분야가 늘 그렇듯, 최신 정보를 파악하거나 초보자가 이 분야에 진입하는 것은 점점 더 어려워지고 있습니다. 특정 하위 문제에 대한 여러 조사 논문이 발표되었지만, 자율주행차를 위한 컴퓨터 비전의 문제, 데이터셋 및 방법에 대한 포괄적인 조사는 아직 발표되지 않았습니다. 이 책은 최첨단 데이터셋과 기술에 대한 조사를 제공함으로써 이러한 격차를 좁히려고 합니다. 우리의 조사는 인식, 재구성, 모션 추정, 추적, 장면 이해, 자율주행을 위한 엔드 투 엔드 학습을 포함한 여러 특정 주제에 대한 역사적으로 가장 관련성 높은 문헌과 현재의 최첨단 기술을 모두 포함합니다. 이러한 목표를 위해, 우리는 KITTI, MOT, Cityscapes를 포함한 여러 도전적인 벤치마킹 데이터셋에서 최첨단 기술의 성능을 분석합니다. 또한, 공개된 문제와 현재의 연구 과제를 논의합니다. 접근성을 높이고 누락된 참조를 수용하기 위해, 우리는 주제와 방법을 탐색하고 추가 정보를 제공하는 웹사이트도 제공합니다.

# 1장

## 서론

1980년대 [168, 170, 661]에 첫 성공적인 시연이 있은 이래로 자율주행차 분야는 큰 발전을 이루었습니다. 그러나 이러한 발전과 야심찬 상업적 목표에도 불구하고, 일반적인 환경에서 완전 자율 내비게이션은 아직 실현되지 않았습니다. 그 이유는 두 가지입니다. 첫째, 복잡하고 동적인 환경에서 작동하는 자율 시스템은 예측 불가능한 상황에 일반화되고 적시에 추론하는 모델을 필요로 합니다. 둘째, 정보에 입각한 결정을 내리려면 정확한 인식이 필요하지만, 기존 컴퓨터 비전 모델 대부분은 여전히 인간의 지각 및 추론 능력보다 뒤떨어집니다.

자율주행에 대한 기존 접근 방식은 크게 모듈형 파이프라인과 모놀리식 엔드 투 엔드 학습 접근 방식으로 분류할 수 있습니다. 두 접근 방식은 그림 1.1에서 개념적 수준에서 대조됩니다. 모듈형 파이프라인은 자율주행의 표준 접근 방식으로, 주로 산업에서 따릅니다. 핵심 아이디어는 고차원 입력에서 저차원 제어 변수로의 복잡한 매핑 함수를 독립적으로 개발, 훈련 및 테스트할 수 있는 모듈로 분해하는 것입니다. 그림 1.1(상단)에서 이러한 모듈은 저수준 인지, 장면 분석, 경로 계획 및 차량 제어를 포함합니다. 그러나 이것은 자율주행 스택을 모듈화하는 한 가지 특정 예이며, 다른 또는 더 세분화된 모듈화도 가능합니다. 기존 접근 방식은 일반적으로 머신러닝(예: 딥러닝 신경망)을 활용하여 저수준 특징을 추출하거나 장면을 개별 구성 요소로 분석합니다. 이와 대조적으로 경로 계획 및 차량 제어는 고전적인 상태 기계, 검색 알고리즘 및 제어 모델에 의해 지배됩니다.

![](./figure/1.1.png)

모듈형 파이프라인의 주요 장점은 시스템의 실패 모드에 대한 통찰력을 얻을 수 있는 감지된 객체 또는 여유 공간 정보와 같은 인간이 해석할 수 있는 중간 표현을 배포한다는 것입니다. 또한, 모듈형 파이프라인 개발은 일반적으로 다른 팀이 운전 문제의 다른 측면에서 동시에 작업하는 회사 내에서 쉽게 병렬화될 수 있습니다. 또한, 시스템에 문제에 대한 첫 번째 원칙과 사전 지식을 통합하는 것이 비교적 쉽습니다. 예를 들어, 플래너에서 명시적으로 시행될 수 있는 교통 법규 또는 향상된 차량 제어로 이어지는 차량 역학에 대한 지식이 있습니다. 보행자의 모습과 같이 손으로 지정하기 어려운 다른 측면은 대규모 주석이 달린 데이터셋에서 학습됩니다.

모듈형 접근 방식의 주요 단점은 인간이 설계한 중간 표현이 일반적으로 안전, 편안함, 목표 도달 시간과 같은 측면을 포함하는 운전 작업에 반드시 최적이지 않다는 것입니다. 또한, 대부분의 모듈은 보조 손실 함수를 사용하여 서로 독립적으로 훈련되고 검증됩니다. 예를 들어 객체 감지 문제를 고려해 보십시오. 장면의 대부분의 객체는 운전 작업과 직접적인 관련이 없지만, 학습 알고리즘은 각 객체의 관련성에 대해 알지 못하므로 신경망에 모든 객체를 동일하게 중요하게 감지하도록 합니다. 따라서 네트워크는 관련 없는 객체에 용량을 낭비하는 반면, 운전 관련 객체를 필요한 정확도로 감지할 수 없습니다. 이는 적절한 중간 표현과 보조 손실 함수를 정의하는 데 어려움이 있음을 보여줍니다.

모듈형 파이프라인에 대한 대안은 딥러닝 신경망과 같은 일반 모델을 사용하여 관찰에서 행동으로의 함수인 정책을 학습하려고 시도하는 엔드 투 엔드 학습 기반 모델입니다. 이 접근 방식은 그림 1.1(하단)에 설명되어 있으며 15장에서 자세히 논의됩니다. 네트워크 매개변수는 교사의 행동을 복제하는 모방 학습을 통해 학습하거나, 세상을 탐색하고 높은 사용자 지정 보상을 얻을 가능성이 있는 행동을 취함으로써 강화 학습을 통해 학습할 수 있습니다. 그러나 강화 학습 접근 방식은 신용 할당 및 보상 형성 문제로 인해 어려움을 겪고, 일반적으로 느리며, 안전에 중요하지 않은 시뮬레이션 환경에서만 적용할 수 있습니다. 반면에 모방 학습은 과적합 문제를 겪고 새로운 시나리오에 쉽게 일반화되지 않습니다. 또한, 전체론적 신경망 기반 접근 방식은 특정 오류가 왜 발생했는지 사용자에게 드러내지 않는 "블랙 박스"로 나타나기 때문에 해석하기 어려운 경우가 많습니다.

이 조사에서는 자율주행차의 지각에 중점을 둡니다. 특히, 모듈형 파이프라인의 지각 관련 모듈과 엔드 투 엔드 학습 기반 접근 방식을 논의합니다. 자율주행 문제의 다른 측면은 관련 조사를 통해 논의됩니다. 예를 들어, Winner et al. [716]은 운전자 보조 시스템에 중점을 두며, 그 구조와 기능을 모두 고려합니다. 마찬가지로 Klette [356]는 비전 기반 운전자 보조 시스템에 대한 개요를 제공합니다. 그들은 지각 문제의 대부분의 측면을 높은 수준에서 설명하지만, 이 조사에서 우리가 추구하는 각 작업의 최첨단 기술에 대한 심층적인 검토를 제공하지는 않습니다. 우리 작업과 보완적으로, Zhu et al. [798]은 지능형 차량의 환경 지각에 대한 개요를 제공하며, 차선 감지, 교통 표지/신호등 인식 및 차량 추적에 중점을 둡니다. 이와 대조적으로, 우리의 목표는 로봇 공학, 지능형 차량 및 컴퓨터 비전 커뮤니티 간의 격차를 해소하고, 이 세 분야의 모든 작업을 포함하는 광범위한 개요 및 비교를 제공하는 것입니다.

이 조사는 다음과 같이 구성됩니다. 먼저, 자율주행의 간략한 역사를 제공한 다음, 카메라 모델 및 교정 기술에 대한 소개를 제공합니다. 그런 다음 지각에 특별히 초점을 맞춘 자율주행 관련 데이터셋에 대한 개요를 제공한 다음, 관련 지각 작업 및 이를 해결하기 위한 최첨단 알고리즘을 조사합니다. 보다 구체적으로, 우리는 객체 감지, 추적, 의미론적 (인스턴스) 분할, 재구성, 모션 추정 및 장면 이해 기술을 검토합니다. 각 장은 문제 정의, 가장 중요한 방법 및 주요 설계 선택에 대한 개요, 가장 인기 있는 데이터셋에서 최고의 성능을 보이는 기술에 대한 정성적 및 정량적 분석, 이 분야의 최첨단 기술에 대한 논의로 시작됩니다. 마지막으로, 이 조사를 마무리하기 전에 자율주행을 위한 최첨단 엔드 투 엔드 모델에 대한 개요를 제공합니다. 탐색을 용이하게 하기 위해, 우리는 또한 조사된 논문을 대화형 그래프 및 추가 정보와 함께 쉽게 접근할 수 있는 방식으로 시각화하는 대화형 온라인 도구1를 제공합니다. 우리는 우리의 조사가 자율 비전 분야의 연구자들에게 유용한 도구가 되고, 이 분야에 대한 철저한 개요를 제공함으로써 초보자의 진입 장벽을 낮추기를 바랍니다.
